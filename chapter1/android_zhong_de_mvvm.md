...
###准备工作
在抽脂之前，我们需要明确一下需要抽取哪些部位。上一章我们讲过，我们可以将有关Model的业务层抽离到ViewModel层。
那么我们可以从私有属性入手，可以将与View没有关联的属性抽出。那么```mAuthTask```和```loginUser```满足这个条件。那么接下来我们可以将有关这两个属性相关的函数和类对象抽离出来，可以满足的函数有辅助函数，功能函数和一个私有内部类。